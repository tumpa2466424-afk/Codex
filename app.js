        ...
        // --- MESSAGING + PROMOTION SYSTEMS ---
        const engagementModuleUrl = new URL('./engagement.js' + new URL(import.meta.url).search, import.meta.url).href;
        const { createMessageSystem, createPromotionSystem } = await import(engagementModuleUrl);
        ...
        const CatalogSystem = {
            ...
            switchTab: async function(tabName) { await UserSystem.ensureAdminModule(); return this.switchTab(tabName); },

            loadData: async function() { await UserSystem.ensureAdminModule(); return this.loadData(); },

            toggleDetails: async function(id) { await UserSystem.ensureAdminModule(); return this.toggleDetails(id); },

            openEditMode: async function(id, event) { await UserSystem.ensureAdminModule(); return this.openEditMode(id, event); },

            openExtrinsicEditMode: async function(id, event) { await UserSystem.ensureAdminModule(); return this.openExtrinsicEditMode(id, event); },

            updateCatalogRow: async function(id, checkboxEl) { await UserSystem.ensureAdminModule(); return this.updateCatalogRow(id, checkboxEl); },

            deleteRow: async function(id, sampleName) { await UserSystem.ensureAdminModule(); return this.deleteRow(id, sampleName); },

            duplicateRow: async function(id, event) { await UserSystem.ensureAdminModule(); return this.duplicateRow(id, event); },

            applyArticleEditorTool: async function(id, part, tool) { await UserSystem.ensureAdminModule(); return this.applyArticleEditorTool(id, part, tool); },

            pullExtrinsicPrice: async function(id, sampleName) { await UserSystem.ensureAdminModule(); return this.pullExtrinsicPrice(id, sampleName); },

            uploadImageToImgBB: async function(id) { await UserSystem.ensureAdminModule(); return this.uploadImageToImgBB(id); },

            cancelEdit: async function(id) { await UserSystem.ensureAdminModule(); return this.cancelEdit(id); },

            saveEdit: async function(id) { await UserSystem.ensureAdminModule(); return this.saveEdit(id); },

            saveExtrinsicEdit: async function(id) { await UserSystem.ensureAdminModule(); return this.saveExtrinsicEdit(id); },
        ...
        toggleNewLotsNotification: async function() {
            ...
            fetchUSDRate: async function() {
                ...
                if (document.getElementById('admin-sec-contracts') && this.adminContractsInstalled) {
            ...
            toggleContractCostDetails: async function() { await this.ensureAdminModule(); return this.toggleContractCostDetails(); },

            updateContractAutoRisk: async function(force = false) { await this.ensureAdminModule(); return this.updateContractAutoRisk(force); },

            calculateContractB2B: async function() { await this.ensureAdminModule(); return this.calculateContractB2B(); },

            initContractB2BAdmin: async function(forceFill = false) { await this.ensureAdminModule(); return this.initContractB2BAdmin(forceFill); },

            saveContractB2BSettings: async function() { await this.ensureAdminModule(); return this.saveContractB2BSettings(); },

            downloadContractB2BTemplate: async function() { await this.ensureAdminModule(); return this.downloadContractB2BTemplate(); },
            ...
            deleteUser: async function(userId) { await this.ensureAdminModule(); return this.deleteUser(userId); },
            ...
            loadActiveSubs: async function() { await this.ensureAdminModule(); return this.loadActiveSubs(); },

            
            // --- НАЧАЛО: АДМИНКА - ОПТОВЫЕ ЗАКАЗЫ ---
            loadWholesaleOrders: async function() { await this.ensureAdminModule(); return this.loadWholesaleOrders(); },
            loadRetailOrders: async function() { await this.ensureAdminModule(); return this.loadRetailOrders(); },
            updateRetailOrderStatus: async function(orderId, newStatus) { await this.ensureAdminModule(); return this.updateRetailOrderStatus(orderId, newStatus); },
            deleteRetailOrder: async function(orderId) { await this.ensureAdminModule(); return this.deleteRetailOrder(orderId); },
            updateOrderStatus: async function(orderId, newStatus) { await this.ensureAdminModule(); return this.updateOrderStatus(orderId, newStatus); },
            deleteWholesaleOrder: async function(orderId) { await this.ensureAdminModule(); return this.deleteWholesaleOrder(orderId); },
        ...
        UserSystem.ensureAdminModule = async function() {
            ...
            if (!adminModulePromise) {
                adminModulePromise = Promise.all([
                    ...
                    .then(([adminModule, adminOpsModule, adminContractsModule, adminCatalogModule, adminStickersModule]) => Promise.all([
                        ...
                        adminOpsModule.installAdminOps({
                            UserSystem,
                            LOCUS_API_URL
                        }),
                        adminContractsModule.installAdminContracts({
                            UserSystem,
                            LOCUS_API_URL
                        }),
                        adminCatalogModule.installAdminCatalog({
                            CatalogSystem,
                            ProductManager,
                            UserSystem,
                            YANDEX_FUNCTION_URL,
